...
async function finishScrapeRun(runId, { status, productsFound, errorMessage, durationMs }) {
  await query(
    `UPDATE scrape_runs
...
async function upsertMasterItem(canonicalName, category = 'Vegetables', aliases = []) {
  const { rows } = await query(
    ...
    for (const p of products) {
      ...
      await client.query(
        `INSERT INTO provider_listings
         ...
         ON CONFLICT (provider_id, master_item_id) DO UPDATE SET
           ...
           scraped_at     = NOW()`,
        [
          p.providerId, p.masterItemId || null, p.name,
          p.masterName || p.name, p.price, p.unit,
          p.available !== false, p.imageUrl || null, p.productUrl || null,
          p.matchMethod || null, runId,
        ]
      )

      // Append to price_history for trend tracking
      if (p.masterItemId) {
        await client.query(
          `INSERT INTO price_history (provider_id, master_item_id, price, unit, available)
           VALUES ($1,$2,$3,$4,$5)`,
          [p.providerId, p.masterItemId, p.price, p.unit, p.available !== false]
        )
      }
    }
...
async function touchProvider(providerId) {
  await query(`UPDATE providers SET last_scraped_at = NOW() WHERE id = $1`, [providerId])
}
