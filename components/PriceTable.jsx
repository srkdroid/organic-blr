...
function getBestWorst(priceMap) {
  ...
  if (!entries.length) return { best: null, worst: null, compareByPerKg: false }
  
  const unitsDiffer = new Set(entries.map(p => p.unit)).size > 1
  const someHavePerKg = entries.some(p => p.price_per_kg != null)
  
  const compareByPerKg = someHavePerKg && unitsDiffer
  const vals = entries.map(p => compareByPerKg ? p.price_per_kg : p.price).filter(v => v != null)
...
function DesktopRow({ item, cart, onToggle }) {
  ...
  const { best, worst, compareByPerKg } = getBestWorst(priceMap)
  const inCart         = cart.has(item.master_item_id)
  
  const bestProviderId = Object.entries(priceMap).find(([, p]) => 
    (compareByPerKg ? p.price_per_kg : p.price) === best
  )?.[0]
  const bestRawPrice = bestProviderId ? priceMap[bestProviderId].price : null
  ...
  return (
    ...
    >
      ...
      {PROVIDER_IDS.map(id => {
        ...
        const metric = compareByPerKg ? p.price_per_kg : p.price
        const cls = metric === best
          ? 'price-best'
          : metric === worst && best !== worst
          ? 'price-worst'
          : 'text-gray-700'
        ...
        return (
          <td key={id} className={`px-2 py-2.5 text-right tabular-nums ${cls}`}>
            ...
            {compareByPerKg && p.price_per_kg && <div className="text-[9px] text-gray-400">₹{p.price_per_kg}/kg</div>}
          </td>
        )
      })}
      <td className="px-3 py-2.5 text-right">
        {bestRawPrice ? (
          <>
            <div className="font-semibold text-emerald-700 tabular-nums">₹{bestRawPrice}</div>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2 mb-2.5">
        ...
        {bestRawPrice && (
          <div className="text-right flex-shrink-0">
            ...
            <div className="font-bold text-emerald-700 tabular-nums text-sm">₹{bestRawPrice}</div>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-1.5">
        {PROVIDER_IDS.map(id => {
          const p       = priceMap[id]
          const metric  = p ? (compareByPerKg ? p.price_per_kg : p.price) : null
          const isBest  = p && metric === best
          const isWorst = p && metric === worst && best !== worst
          ...
          return (
            ...
            >
              ...
              {compareByPerKg && p.price_per_kg && <div className="text-[8px] text-gray-400 truncate">₹{p.price_per_kg}/kg</div>}
            </div>
          )
        })}
      </div>
