...
export function evaluateSubset(subsetIds, cartItems, allItems) {
  ...
  for (const cartItem of cartItems) {
    ...
    // Find cheapest provider IN THE SUBSET that has this item (matching the requested unit/variant)
    ...
    if (bestProvider) {
      ...
      const { isExact } = getPriceForVariant(
        (item.prices || []).filter(pr => pr.provider_id === bestProvider && pr.available && pr.price),
        cartItem.unit
      )
      const unitLabel = bestVariantObj.unit
        ? (isExact ? bestVariantObj.unit : `~${bestVariantObj.unit}`)
        : ''
