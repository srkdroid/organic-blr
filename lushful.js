 ...
 * Run alone : node scraper/scrapers/lushful.js
 */
...
const CFG = PROVIDERS.lushful

const COLLECTIONS = [
  CFG.vegetablesApiUrl,
  CFG.fruitsApiUrl,
  'https://lushful.org/collections/organic-vegetables/products.json?limit=250',
  'https://lushful.org/collections/organic-fruits/products.json?limit=250',
].filter(Boolean)

async function scrape() {
  logger.info('[LU] Starting Lushful scrape via Shopify JSON')
  return withRetry(
    () => scrapeShopify({ providerId: CFG.id, collectionsJson: COLLECTIONS }),
    { retries: 3, label: 'LU full scrape' }
  )
}
