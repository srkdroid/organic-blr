...
async function runScraper({ id, module: mod }, masterItems) {
  ...
  try {
    ...
    if (!rawProducts || rawProducts.length === 0) {
      ...
      await db.finishScrapeRun(runId, { status: 'partial', productsFound: 0, durationMs: Date.now() - startedAt })
      return
    }
    ...
    for (const p of normalised) {
      ...
      if (existing) {
        ...
        const newId = await db.upsertMasterItem(p.masterName, p.category || 'Vegetables', [p.name])
        p.masterItemId = newId
        masterItems.push({ id: newId, canonical_name: p.masterName, category: p.category, aliases: [p.name] })
      }
    }

    // Save only confirmed fresh produce with a master item ID
    const toSave = normalised.filter(p => p.isFreshProduce !== false && p.masterItemId)
    await db.saveListings(toSave, runId)
    await db.touchProvider(id)

    const duration = Date.now() - startedAt
    await db.finishScrapeRun(runId, { status: 'success', productsFound: toSave.length, durationMs: duration })
    logger.info(`[Scheduler] ✓ ${id} — ${toSave.length} products saved in ${(duration / 1000).toFixed(1)}s`)

  } catch (err) {
    const duration = Date.now() - startedAt
    logger.error(`[Scheduler] ✗ ${id} failed`, { error: err.message })
    await db.finishScrapeRun(runId, {
      status: 'failed', productsFound: 0,
      errorMessage: err.message, durationMs: duration,
    })
  }
}
...
async function runAll() {
  ...
  const lightPromises = LIGHTWEIGHT.map(s => limit(() => runScraper(s, masterItems)))
